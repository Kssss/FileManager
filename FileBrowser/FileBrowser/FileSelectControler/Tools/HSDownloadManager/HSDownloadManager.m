...
// 文件的存放路径（caches）
#define HSFileFullpath(url) [HSCachesDirectory2 stringByAppendingPathComponent:HSFileName(url)]
...
- (void)createCacheDirectory
{
    ...
    if (![fileManager fileExistsAtPath:HSCachesDirectory2]) {
        [fileManager createDirectoryAtPath:HSCachesDirectory2 withIntermediateDirectories:YES attributes:nil error:NULL];
    }
}
...
- (void)deleteAllFile
{
    ...
    if ([fileManager fileExistsAtPath:HSCachesDirectory2]) {
        // 删除沙盒中所有资源
        [fileManager removeItemAtPath:HSCachesDirectory2 error:nil];
        // 删除任务
